...
class DomainManager<T extends Record<string, any>> {
    ...
    getDomain(
        ...
        if (dataType === 'string') {
            return [...new Set(this.data.map(accessor as DataAccessor<T, string>))];
        } else {
    ...
    getNumericalDomain(
        ...
        if (dataType === 'date' && minValue instanceof Date && maxValue instanceof Date) {
    ...
    private padDomain(domain: [number, number], padding: number): [number, number] {
